...
function analyzeResults(projects: ProjectVersion[]): VersionCalculationResult {
  const changedProjects = projects.filter(p => p.versionChanged)
  const hasChanges = changedProjects.length > 0

  const summary = generateSummary(projects, changedProjects)
